...
class FtdiPort:
    def __init__(self, port, baud):
        ...
        if port != ESPLoader.DEFAULT_PORT:
           ...
           self._device.open(0x0403, 0x6015)
    ...
    def enter_bootloader(self, mode, esp32r0_delay):
        if mode != 'no_reset':
            # 0x20 = CBUS BitBang
            # Set CBUS2 low
            self._device.set_bitmode(0b01000000, ftdi.Ftdi.BITMODE_CBUS)
...
class PySerialPort:
    ...
    def reset(self):
        self._serial.setRTS(True)  # EN->LOW
        time.sleep(0.1)
    ...
    def enter_bootloader(self, mode, esp32r0_delay):
        ...
        if mode != 'no_reset':
            self._serial.setDTR(False)  # IO0=HIGH
            self._serial.setRTS(True)   # EN=LOW, chip in reset
            time.sleep(0.1)
            if esp32r0_delay:
                ...
                # for a longer period
                time.sleep(1.2)
            self._serial.setDTR(True)   # IO0=LOW
            self._serial.setRTS(False)  # EN=HIGH, chip out of reset
            if esp32r0_delay:
            ...
            time.sleep(0.05)
            self._serial.setDTR(False)  # IO0=HIGH, done
...
class ESPLoader(object):
    ...
    def __init__(self, port=DEFAULT_PORT, baud=ESP_ROM_BAUD, use_ftdi=False):
        ...
        """
        if not isinstance(port, FtdiPort) and not isinstance(port, PySerialPort):
            if use_ftdi:
                print("Using ftdi")
                self._port = FtdiPort(port=port, baud=baud)
            else:
                self._port = PySerialPort(port=port, baud=baud)
        else:
            self._port = port
    ...
    @staticmethod
    def detect_chip(port=DEFAULT_PORT, baud=ESP_ROM_BAUD, use_ftdi=False, connect_mode='default_reset'):
        ...
        """
        detect_port = ESPLoader(port, baud, use_ftdi)
    ...
    def _connect_attempt(self, mode='default_reset', esp32r0_delay=False):
        ...
        self._port.enter_bootloader(mode, esp32r0_delay)
    ...
    def erase_flash(self):
        oldtimeout = self._port.get_timeout()
        # depending on flash chip model the erase may take this long (maybe longer!)
        self._port.set_timeout(128)
        try:
            self.check_command("erase flash", self.ESP_ERASE_FLASH)
        finally:
            self._port.set_timeout(oldtimeout)
...
def main():
    ...
    parser.add_argument(
            '--ftdi', '-f',
            help = 'Use ftdi cbus 2 gpio to enter the bootloader',
            action='store_true')
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        if args.chip == 'auto':
            esp = ESPLoader.detect_chip(args.port, initial_baud, args.ftdi, args.before)
        else:
            chip_class = {
                'esp8266': ESP8266ROM,
                'esp32': ESP32ROM,
            }[args.chip]
            esp = chip_class(args.port, initial_baud, args.ftdi)
