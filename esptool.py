...
class ESPROM:
    ...
    def __init__(self, port, baud = ESP_ROM_BAUD, use_ftdi=False):
        if not use_ftdi:
            if not port:
                port = '/dev/ttyUSB0'
    ...
    def connect(self):
        ...
        for _ in xrange(4):
            self._port.enter_bootloader()

            self._port.set_timeout(0.3) # worst-case latency timer should be 255ms (probably <20ms)
            for _ in xrange(4):
                try:
...
if __name__ == '__main__':
    ...
    if args.operation not in ('image_info','make_image','elf2image'):
        esp = ESPROM(args.port, args.baud, args.ftdi)
        esp.connect()
